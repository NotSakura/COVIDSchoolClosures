...
Lastly is the comparison between having a in person class and virtual class. If it wasn't clear from the previous three graphs (@fig-EnrollAll, @fig-EnrollKinder and @fig-Enroll2019) that in-person learning results in higher enrollment rates, the negative correlation with virtual environment does. In @fig-EnrollVirtual we see that the line of best fit is negative compared to the previous graphs. This means that as more schools went virtual the more the enrollment rates dropped. This supports the papers hypothesis that students are more willing to learn in a environment where they are able to physically interact with people. Part of this might be due to the fact that a lot of students, when in person feel more inclined to work harder as the task they are doing is not as easy to ignore compared to a virtual environment where they can simply log off. 
